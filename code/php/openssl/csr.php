...
// CA
$password = '123456';
$ca_priv = openssl_pkey_new($config);
$ca_csr = openssl_csr_new($dn, $ca_priv, $config);
$ca_cert = openssl_csr_sign($ca_csr, null, $ca_priv, 365);
...
//openssl_pkcs12_read ($ca_pfx, $certs, $password);
//var_dump($certs);
echo "\n\n";
...
//$cert = openssl_csr_sign($csr_out, null, $priv, 365);
// CA 签证书
...
openssl_csr_export($csr, $csr_out) and var_dump('CSR', $csr_out);
openssl_x509_export($cert, $certout) and var_dump('Certificate', $certout);
openssl_pkey_export($priv, $pkeyout, "mypassword", $config) and var_dump('Private', $pkeyout);
